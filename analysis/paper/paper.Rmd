...
# library(buckshot) # Or use devtools::load_all('.', quiet = T) if your code is in script files, rather than as functions in the `/R` diretory
```
...
satellite-linked time-depth recorders (TDRs). In these models, haul-out
...
logistical constraints on tagging fieldwork and the operational life of
...
mounted satellite tags provide some benefits over flipper mounted tags (such as
increased satellite transmittal rates), but these typically are lost during
...
In this study, we use available ARGOS-linked satellite tagging records in a
...
This article is structured as follows. First, we describe satellite telemetry
